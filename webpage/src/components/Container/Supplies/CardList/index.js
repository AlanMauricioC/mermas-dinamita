		...
		return (
			<Grid container direction="row" alignItems="center" spacing={2} className={classes.root}>
				...
				<DialogUpdateSupply
					...
					supply={this.state.supplyToUpdate}
				/>
				<DialogDeleteSupply
...
function mapStateToProps(state) {
	switch (state.supplies.type) {
		case SUPPLY_SELECTOR.NONE:
			return { supplies: state.supplies.supplies.filter((supply) => supply.name.includes(state.supplies.qry)) };
		case SUPPLY_SELECTOR.OK:
			return {
				supplies: state.supplies.supplies.filter(
					(supply) =>
						supply.name.includes(state.supplies.qry) &&
						supply.quantity >= supply.avg &&
						supply.quantity < supply.max
				)
			};
		case SUPPLY_SELECTOR.WARNING:
			return {
				supplies: state.supplies.supplies.filter(
					(supply) =>
						supply.name.includes(state.supplies.qry) &&
						supply.quantity <= supply.avg &&
						supply.quantity > supply.min
				)
			};
		case SUPPLY_SELECTOR.ERROR:
			return {
				supplies: state.supplies.supplies.filter(
					(supply) =>
						supply.name.includes(state.supplies.qry) &&
						(supply.quantity < supply.min || supply.quantity > supply.max)
				)
			};
		default:
			return { supplies: state.supplies.supplies.filter((supply) => supply.name.includes(state.supplies.qry)) };
